...
#         label = request.POST.get('label')
#         id_album = str(uuid.uuid4())
#         cursor.execute(f'insert into album values (\'{id_album}\', \'{judul}\', 0, \'{label}\', 0)')
#         connection.commit()
#         return redirect('album_royalti:list_edit_album')
    
#     cursor.execute(f'select id, nama from label')
#     list_label = cursor.fetchall()
...
#     else:
#         id_artist = request.POST.get('artist')
#         cursor.execute(f'select id_pemilik_hak_cipta from artist where id = \'{id_artist}\'')
#         id_pemilik_hak_cipta_artist = cursor.fetchone()
#     return id_artist, id_pemilik_hak_cipta_artist

# def insert_konten(id_song, judul, date_now, year_now, durasi):
#     cursor.execute(f'insert into konten values (\'{id_song}\', \'{judul}\', \'{date_now}\', \'{year_now}\', \'{durasi}\')')

# def insert_song(id_song, id_artist, album_id):
#     cursor.execute(f'insert into song values (\'{id_song}\', \'{id_artist}\', \'{album_id}\', 0, 0)')

# def insert_songwriter_write_song(id_song, songwriters):
#     for songwriter in songwriters:
#         cursor.execute(f'select id_pemilik_hak_cipta from songwriter where id = \'{songwriter}\'')
#         id_pemilik_hak_cipta_songwriter = cursor.fetchone()
#         cursor.execute(f'insert into royalti values (\'{id_pemilik_hak_cipta_songwriter[0]}\', \'{id_song}\', 0)')
#         cursor.execute(f'insert into songwriter_write_song values (\'{songwriter}\', \'{id_song}\')')

# def insert_genres(id_song, genres):
#     for genre in genres:
#         cursor.execute(f'insert into genre values (\'{id_song}\', \'{genre}\')')

# def insert_royalti(id_pemilik_hak_cipta, id_song):
#     cursor.execute(f'insert into royalti values (\'{id_pemilik_hak_cipta}\', \'{id_song}\', 0)')

# def insert_royalti_label(album_id, id_song):
#     cursor.execute(f'select id_label from album where id = \'{album_id}\'')
#     id_label = cursor.fetchone()
#     cursor.execute(f'select id_pemilik_hak_cipta from label where id = \'{id_label[0]}\'')
#     id_pemilik_hak_cipta_label = cursor.fetchone()
#     cursor.execute(f'insert into royalti values (\'{id_pemilik_hak_cipta_label[0]}\', \'{id_song}\', 0)')

# def update_album(album_id, durasi):
#     cursor.execute(f'select jumlah_lagu, total_durasi from album where id = \'{album_id}\'')
#     album_saat_ini = cursor.fetchone()
#     new_total_durasi = int(album_saat_ini[1]) + int(durasi)
#     new_jumlah_lagu = int(album_saat_ini[0]) + 1
#     cursor.execute(f'UPDATE album SET jumlah_lagu = {new_jumlah_lagu}, total_durasi = {new_total_durasi} WHERE id = \'{album_id}\'')

# def get_artist_records():
#     cursor.execute(f'select id, email_akun, id_pemilik_hak_cipta from artist')
#     records_artist = cursor.fetchall()
#     for i in range(len(records_artist)):
#         cursor.execute(f'select nama from akun where email = \'{records_artist[i][1]}\'')
#         records_artist[i] = records_artist[i] + cursor.fetchone()
#     return records_artist

# def get_songwriter_records():
#     cursor.execute(f'select id, email_akun, id_pemilik_hak_cipta from songwriter')
#     records_songwriter = cursor.fetchall()
#     for i in range(len(records_songwriter)):
#         cursor.execute(f'select nama from akun where email = \'{records_songwriter[i][1]}\'')
#         records_songwriter[i] = records_songwriter[i] + cursor.fetchone()
...
#     cursor.execute(f'select judul from album where id = \'{album_id}\'')
#     judul_album = cursor.fetchone()
#     return judul_album
...
#     nama_artist = ""
#     if isArtist == "True":
#         cursor.execute(f'select email_akun from artist where id = \'{idArtist}\'')
#         email_artist = cursor.fetchone()
#         cursor.execute(f'select nama from akun where email = \'{email_artist[0]}\'')
#         nama_artist = cursor.fetchone()
#     return nama_artist
...
#     nama_songwriter = ""
#     if isSongwriter == "True":
#         cursor.execute(f'select email_akun from songwriter where id = \'{idSongwriter}\'')
#         email_songwriter = cursor.fetchone()
#         cursor.execute(f'select nama from akun where email = \'{email_songwriter[0]}\'')
#         nama_songwriter = cursor.fetchone()
...
#     cursor.execute(f'select * from album where id_label = \'{id_label}\'')
#     records_album = cursor.fetchall()
...
#     cursor.execute(f'SELECT DISTINCT id_album FROM SONG WHERE id_artist = \'{id_artist}\'')
#     list_album_id_artist = cursor.fetchall()
...
#         cursor.execute(f'SELECT id, judul, id_label, jumlah_lagu, total_durasi from album where id = \'{id_album[0]}\'')
#         album_record = cursor.fetchone()
#         cursor.execute(f'SELECT nama FROM LABEL WHERE id = \'{album_record[2]}\'')
...
#     list_song_id_songwriter = []
#     cursor.execute(f'SELECT id_song FROM songwriter_write_song WHERE id_songwriter = \'{id_songwriter}\'')
#     list_song_id_songwriter = cursor.fetchall()

#     list_album_id_songwriter = []
#     for song in list_song_id_songwriter:
#         cursor.execute(f'SELECT id_album FROM SONG WHERE id_konten = \'{song[0]}\'')
#         list_album_id_songwriter.append(cursor.fetchone())
